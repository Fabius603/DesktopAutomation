...
namespace ImageCapture.DesktopDuplication
{
    public class DesktopDuplicator
    {
        ...
        private void RetrieveCursorMetadata(DesktopFrame frame)
        {
            ...
            if (updatePosition)
            {
                pointerInfo.Position = new RawPoint(frameInfo.PointerPosition.Position.X, frameInfo.PointerPosition.Position.Y);
            ...
            try
            {
                unsafe
                {
                    ...
                    {
                        mDeskDupl.GetFramePointerShape(frameInfo.PointerShapeBufferSize, (nint)ptrShapeBufferPtr, out pointerInfo.BufferSize, out pointerInfo.ShapeInfo);
                    }
                }
            }
            catch (SharpDXException ex)
        ...
        private int GetWidth(RawRectangle rect)
        {
            return rect.Right - rect.Left;
        }

        private int GetHeight(RawRectangle rect)
        {
        ...
        private void ProcessFrame(DesktopFrame frame)
        {
            ...
            FinalImage = new Bitmap(GetWidth(mOutputDesc.DesktopBounds), GetHeight(mOutputDesc.DesktopBounds), PixelFormat.Format32bppRgb);
            var boundsRect = new Rectangle(0, 0, GetWidth(mOutputDesc.DesktopBounds), GetHeight(mOutputDesc.DesktopBounds));
            // Copy pixels from screen capture Texture to GDI bitmap
            var mapDest = FinalImage.LockBits(boundsRect, ImageLockMode.WriteOnly, FinalImage.PixelFormat);
            var sourcePtr = mapSource.DataPointer;
            var destPtr = mapDest.Scan0;
            for (int y = 0; y < GetHeight(mOutputDesc.DesktopBounds); y++)
            {
                // Copy a single line 
                Utilities.CopyMemory(destPtr, sourcePtr, GetWidth(mOutputDesc.DesktopBounds) * 4);
